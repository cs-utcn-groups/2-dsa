...
int getNrOfNeighborsOfVertexFromList(int v)
{
    if(v<nrOfVertexes)
    {
        int nrOfNeighbors=0;
        nodeL *currentNode=(nodeL*)malloc(sizeof(nodeL));
        currentNode=adjList[v].first;
...
int *getAllNeighborsOfVertexFromList(int v)
{
    if(v<nrOfVertexes)
    {
        int nrOfNeighbors = getNrOfNeighborsOfVertexFromList(v);
        if(nrOfNeighbors>0)
        {
            int *neighbors = (int *) malloc(sizeof(int) * nrOfNeighbors);
            nodeL *currentNode=(nodeL*)malloc(sizeof(nodeL));
            ...
            {
                neighbors[i]=currentNode->letter-'A';
                currentNode=currentNode->next;
            }
        return neighbors;
        }
